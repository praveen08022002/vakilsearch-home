        ...
        {data?.map((item, index) => (
          <div key={index} className="flex flex-col max-md:gap-[12px]">
            ...
            <div className="flex flex-col md:mt-[50px] md:gap-[12px]">
              ...
              <div className="text-sm md:text-[18px] text-left text-[#434A53] font-normal md:leading-[30px] max-w-[270px] md:max-w-[450px]">
                {parse(item?.excerpt?.rendered)}
              </div>
              {item?.underLineText && (
                <Link
                  href={`${item?.link}`}
                  className="text-lg md:text-[18px] cursor-pointer text-left text-[#434A53] font-normal leading-[30px] underline pt-4 md:pt-0"
                >
                  Read More
                </Link>
              )}
