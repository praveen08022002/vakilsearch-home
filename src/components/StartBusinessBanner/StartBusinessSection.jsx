...
const StartBusinessSection = ({ data }) => {
    return (
        <div className="flex flex-col relative w-full bg-[#022B50] md:mt-[100px] items-start gap-[25px] py-[40px] px-[15%]">
            <p className="w-full text-[36px] text-left weight-[700px] leading-[48px] text-[#ffffff]">{data?.startBusinessSection?.title}</p>
            <p className="w-full text-[18px] text-left font-normal weight-[400px] leading-[26px] text-[#ffffff]">{data?.startBusinessSection?.desc}</p>
            ...
            className="absolute top-[-40px] right-[200px] bottom-0" />
        </div>
    )
}
